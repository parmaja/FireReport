...
unit FR_Interpreter deprecated;
...
uses
  Contnrs, Classes, SysUtils, Graphics,
...
procedure DoFuncId; forward;
  ...
  begin
    ...
    MemoErr.Add('Error at: ' + IntToStr(i + 1) + '/' + IntToStr(j - 1) + ': ' + s);
  end;
  ...
  begin
    ...
    if CharInSet(buf^[cur], ['0'..'9']) then
      while CharInSet(buf^[cur], ['0'..'9']) and (cur < len) do Inc(cur)
    else Error := True;
  end;
  ...
  1:DoCommand;
    ...
    begin
      ...
      AddError('You need to add ";" or "end"');
  end;
  ...
  1:DoCommand;
    ...
    begin
     ...
     else AddError('You need to add ";" or "until"');
  end;
  ...
  begin
    ...
    else AddError('You need to add "do"');
  end;
  ...
  begin
    ...
    begin
      Inc(cur);
      while CharInSet(buf^[cur], ['0'..'9', '_', '.', 'A'..'Z', 'a'..'z']) do Inc(cur);
      if buf^[cur] = '(' then
        Error := True;
      if buf^[cur] = '[' then
      begin
  ...
  1:  DoExpression;
      ...
      else AddError('You need to add "," or ")"');
    end;
  end;
