...
uses
  ...
  ExtCtrls, Buttons, StdCtrls, Menus, System.Types, System.UITypes,
  FR_Consts;
...
uses
  FR_Classes, Printers, FR_Printer, FR_Search, Registry, FR_PrintDialog, FR_Utils;
...
begin
  if EMFPages = nil then Exit;
  Label1.Caption := 'Page ' + IntToStr(CurPage) + '/' +
...
begin
  if EMFPages = nil then Exit;
  OpenDialog.Filter := 'Report file' + ' (*.frp)|*.frp';
  with OpenDialog do
   if Execute then
...
begin
  ...
  if TfrEMFPages(EMFPages).Count > 1 then
    if MessageBox(0, 'Remove this page?', 'Confirm',
